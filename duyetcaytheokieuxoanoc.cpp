...
void treeDisplaySpiralOrder(Tree &T)
{
    ...
    while(!q.empty())
    {
        ...
        if (dir == 0) 
        {
            ...
            cout << q.front().F->infor << ' ';
        }
        else
        {
            st.push(q.front().F->infor);
        }
        q.push(mp(q.front().F->right, 1-dir));
        q.push(mp(q.front().F->left, 1-dir));
        q.pop();
    }
    while(st.size())
    {
